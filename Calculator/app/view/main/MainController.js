Ext.define('ExtCalculator.view.main.MainController', {

    extend: 'Ext.app.ViewController',
    alias: 'controller.calculatorcontroller',
    views: ['ExtCalculator.view.main.Main'],
    models: ['Main'],
    ...
    onClickOp: function (btn) {
        ...
        if (this.state.selectedOperator != null && this.state.numberClicked === true) {
            ...
            var lastResult = Math.round(result * 100) / 100;

            vm.set('operation',op1 + ' ' + this.state.selectedOperator + ' ' + op2 + ' = ' + lastResult);
            vm.set('display', lastResult);
            this.state.selectedOperator = null;
        }
        if (btn.text != '=') {
            this.state.operatorClicked = true;
            this.state.op1 = vm.get('display');
            vm.set('operation',this.state.op1 + ' ' +btn.text);
        }
    ...
    onClickDot: function (btn) {
        ...
        if (this.state.numberClicked === false && this.state.operatorClicked === true) {
            vm.set('display', '0.');
